...
class PrefixTree:
    ...
    def add_tenants(self, tenants: List[str], time_s: float) -> None:
        ...
        with self.lock:
            for tenant in tenants:
                ...
                self._insert_locked("", tenant, time_s)
    ...
    def insert(self, text: str, tenant: str, time_s: float) -> None:
        ...
        """
        with self.lock:
            self._insert_locked(text, tenant, time_s)

    def insert_many(self, items: List[Tuple[str, str, float]]) -> None:
        """
        Insert multiple texts while holding the tree lock once.

        Args:
            items: List of (text, tenant, time_s) tuples.
        """
        with self.lock:
            for text, tenant, time_s in items:
                self._insert_locked(text, tenant, time_s)

    def _insert_locked(self, text: str, tenant: str, time_s: float) -> None:
        if tenant not in self.tenant_to_char_count:
            logger.debug(
